...
void Cmd_Roll_f(gentity_t* ent) {
	...
	if (trap_Argc() != 2) {
		trap_SendServerCommand(ent - g_entities, "print \"^1Использование: ^2/roll ^3<макс. число>.\n^1Пример: ^2/roll ^320\n\"");
		return;
	}
	...
	if (StringIsInteger(arg1) == qfalse) {
		trap_SendServerCommand(ent - g_entities, "print \"Аргумент должен быть целым числом.\n\"");
		return;
	}
	...
	if (max_value < 2) {
		trap_SendServerCommand(ent - g_entities, "print \"Максимальное значение должно быть не менее 2.\n\"");
		return;
	}

	int result = roll_dice(max_value);

	trap_SendServerCommand(-1, va("chat \"^3<Бросок кости> %s^2 выбросил ^3%d^2 из ^3%d\n\"",
...
#define MAX_WORDED_EMOTES 52 //132 галаксирп 52 в списке
...
void Cmd_Emote_f(gentity_t* ent)
{
	...
	if (trap_Argc() < 2) {
		trap_SendServerCommand(ent - g_entities, va(
			"print \"Использование: /anim <id анимации от 0 до %d>\n\"", MAX_ANIMATIONS - 1));
		return;
	}
	...
	if (Q_stricmp(anim_id, "list") == 0)
	{
		if (trap_Argc() == 3) {
			...
			if (page > -1) {
				...
				trap_SendServerCommand(ent - g_entities, "print \"Такой страницы эмоутов не существует.\n\"");
				return;
			}
		}
		else {
			show_animation_list(ent, 0, 3);
			trap_SendServerCommand(ent - g_entities, "print \"^3Страница 1/2. Чтобы посмотреть остальные анимации, напишите /anim list 2\n\"");
			return;
		}
	}
	else if (Q_stricmp(anim_id, "stop") == 0 && !(ent->client->pers.player_statuses & (1 << 6))) {
		ent->client->pers.player_statuses &= ~(1 << 1); // убрать флаг "эмоция"
	...
	if (anim_id_int < 0 || anim_id_int >= MAX_ANIMATIONS) {
		trap_SendServerCommand(ent - g_entities, va(
			"print \"ID анимации должен быть от 0 до %d\n\"", MAX_ANIMATIONS - 1));
		return;
	}

	if (ent->client->ps.forceHandExtend == HANDEXTEND_KNOCKDOWN) {
		trap_SendServerCommand(ent->s.number, "print \"Нельзя использовать эмоуты, когда вы сбиты с ног.\n\"");
		return;
	}
	...
	if (StringIsInteger(anim_id))
	{
		...
		for (int i = 0; i < MAX_WORDED_EMOTES; i++) {
			if (Q_stricmp(anim_id, animations[i].animation_name) == 0) {
				...
				ent->client->ps.forceHandExtendTime = level.time + 1000000;
				ent->client->pers.player_statuses |= (1 << 1); // установить флаг "в эмоуте"
				return;
			}
		}
	}
...
void help_up(gentity_t* ent, gentity_t* target, qboolean isSuicide) {
	if ((can_player_get_up(ent, target)) || isSuicide) {
		...
		target->client->ps.eFlags &= ~EF_INVULNERABLE; // убрать флаг неуязвимости
...
// пытаюсь сделать телепорт // скопировал из ОпенРП
