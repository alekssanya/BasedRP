...
#define	MIN_NPC_SPEED	16.0f
	...
	{//racc - we're in a private duel.
		...
		else
		{
			...
			if (subLen >= 640) // Сократил Расстояние для разрыва дуэли с 1024 до 640
...
//	BotTestAAS(ent->r.currentOrigin);
	...
	//tyt
	if (ent->client->pers.player_statuses & (1 << 6)) {
		// Игрок повержен — принудительно поддерживаем анимацию knockdown
		ent->client->ps.forceHandExtend = HANDEXTEND_KNOCKDOWN;
		ent->client->ps.forceDodgeAnim = 0; // или ent->client->pers.downedAnim, если ты сохраняешь
